...
const DateCard = ({ item, checked, colorIndex, onToggle }: DateCardProps) => {
  ...
  return (
    <button
      ...
      aria-label={`${item.date} — ${item.event} — ${checked ? "coché" : "non coché"}`}
    ...
    >
      ...
      <div className="relative flex-shrink-0 mt-0.5">
        <div
          ...
          style={{ background: colors.checkBg, opacity: checked ? 0.5 : 0.85 }}
        >
          {item.type === "range" ? (
