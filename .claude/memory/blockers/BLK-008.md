...
# BLK-008 — firstTryScore = 27/27 quelle que soit la performance
...
if (!answeredCodes.has(activeCountry.code)) {
  setFirstAttemptCodes(prev => { const n = new Set(prev); n.add(code); return n; });
}
```
...
setFailedCodes(prev => { const n = new Set(prev); n.add(code); return n; });
```
